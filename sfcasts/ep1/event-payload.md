...
with the search term data here. Now we need to do those. We need to communicate when
the search term changed up to catalog. So we have a search bar right here when the
...
event itself. Another reason I'm not going to move it up. The search term up to
...
All right. So what we need to do in search by then is we need to decide we're going
to going to emit a custom event when the search term changes. So do that. I'm
actually going to add an add input = and here I'm going to say on, that'd be a new
method. Now, as you mentioned earlier, when you had V model, what that, one of the
things that does behind the scenes is it adds an add input which, uh, sets the search
term to this inputs value. In this case, we're actually adding a second add input. So
the input behind the model is still going to update the search term data, but then we
can also run our own custom code after it's update the data, which is pretty cool. So
down here, I'm gonna add a methods and what a new ethic called on input. And inside
of here, we're going to dispatch, we're going to emit an event which we do via
this.dollar sign emit. And let's call this search search products. If we want it to,
...
happened, we need to also pass the, what the search term is. So we'll say term,
this.search term.
...
have it. So we don't want to, when the search term changes, we don't want to actually
...
the search term. And then we'll use these two pieces of data to actually figure out
what subset of products to show. Now you might be yelling, Hey Ryan, you have just
duplicated the search term data. You have searched term in the search bar and you
have search term inside of the catalog. You said never to do that. You said that you
should have a search term up here and probably pass this as a proper down to search
bar. That does very good point. However, I'm doing it on purpose. It's a sick with me
with this. The search term, instead of search bar is really internal state just to
help this component do its job. The real end result of this component is that a
dispatching of events with the new search term. If we're using search bar, we don't
really care that it has this search term data. We only care that it emits this event.
And then in catalog, we're going to listen to that event and kind of the same thing
catalog also needs to have also is going to have a search term just so that it can
...
the search term on search bar will update instantly, but there'll be a slight delay
...
products. We have search term, let's update the search term when that changes. So as
a reminder, we just, this pageant event called search products. So what I'm going to
do is go up to my search bar up here. There we go. And we'll say that search dash
products = and we'll say on search products, which is a method that we now need to
create. So down here,

[inaudible]

at the bottom, I'll add methods and then on search products now, because this is
...
event, we added a turn key. We can use that. So we can say this, that search term =
event that term, and that should be enough to do it. So let's just check our work
...
I'm going to change this to filtered products. I'll copy that name. And now I go down
and do a computer property above created, I'll say, computed filter products. And
that function, I'll say, if not this stuff, search term, then we're going to return
this.product. So just the normal array of products. If there is a search term, then
what we can do is return this stop products that filter and pass that a call back. I
use the->function that will take the individual product as we're looping over it. And
...
say product that name, that to lower

case

that includes this search term, that to lowercase, there we go. So basically it's
gonna loop over all the products and get rid of that stomach colon.

It's going to loop over all the products and for each product, if product.name, that
to lowercase includes this, that search term.to lowercase. Then it's going to be
